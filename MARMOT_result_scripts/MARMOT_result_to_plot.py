...
################
redmine_project = '3207'
# HD149027: '3207'; HD189733: '3155'; HAT-P-2: '3204'
datapath = '/mnt/e/science/RM_effect/data/HD_149026_popultest/'
error_meth = 'ccf_error'  # 'order_scatter'
...
def read_marmot_singlefile(redmine_id, excluded_orders):
    dates_rv_array = []
    with open(os.path.join(datapath, 'RVs_ID{}_singleord_marmot_ccfopt.txt'.format(redmine_id)), 'r') as in_sing_file:
...
def calc_weighted_mean(redmine_id, rvs_array, error_method='order_scatter', clipping=False, sigma_coeff=1.0):
    ...
    with open(os.path.join(datapath, 'RVs_ID{}_newweighted_marmot_ccfopt.txt'.format(redmine_id)), 'w') as out_weight_file:
