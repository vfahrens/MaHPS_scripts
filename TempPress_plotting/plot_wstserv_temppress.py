...
path = u'/mnt/c/Users/Bananaking/Documents/Vanessa/PhDThesis/plotting_stuff/logs/'
# skip = 1  # plottet jeden Datenpunkt
skip = 1  # ueberspringt die angegebende Anzahl von Datenpunkten umd das Program schneller zu machen

# define the files from which the logged values should be read
## long-term with HLE2, JunJul
# ptc10new_logs = 'ptc10new_20210620-0731.log.gz'
# ptc10old_logs = 'ptc10old_20210620-0731.log.gz'
# wut_room_logs = 'wut_foces-room_20210620-0731.log.gz'
# cpg2500_logs = 'cpg2500_20210620-0731.log.gz'
## long-term with HLE2, MarApr
# ptc10new_logs = 'ptc10new_20210310-0419.log.gz'
# ptc10old_logs = 'ptc10old_20210310-0419.log.gz'
# wut_room_logs = 'wut_foces-room_20210310-0419.log.gz'
# cpg2500_logs = 'cpg2500_20210310-0419.log.gz'
## when there was no temp control
ptc10new_logs = 'ptc10new_20210906-0909.log.gz'
ptc10old_logs = 'ptc10old_20210906-0909.log.gz'
wut_room_logs = 'wut_foces-room_20210906-0909.log.gz'
cpg2500_logs = 'cpg2500_20210906-0909.log.gz'
##### old from SPIE paper
# ptc10new_logs = 'ptc10new_20201110-19.log.gz'
# ptc10old_logs = 'ptc10old_20201110-19.log.gz'
# wut_room_logs = 'wut_foces-room_20201110-19.log.gz'
# cpg2500_logs = 'cpg2500_20201110-19.log.gz'

# Definiert den anzuzeigenden Zeitbereich
## long-term with HLE2, JunJul
# t_end = datetime.strptime('2021-07-30 00:00:00', '%Y-%m-%d %H:%M:%S')
# t_delta = dt.timedelta(days=37.0)
## long-term with HLE2, MarApr
# t_end = datetime.strptime('2021-04-19 12:00:00', '%Y-%m-%d %H:%M:%S')
# t_delta = dt.timedelta(days=38.5)
## when there was no temp control
t_end = datetime.strptime('2021-09-08 08:00:00', '%Y-%m-%d %H:%M:%S')
t_delta = dt.timedelta(days=1.333)
##### old from SPIE paper
...
# #### for the Temp plot
# plot1_x = time_messptc10
# plot1_y = temp_messptc10_3B
# plot2_x = time_messptc10
# plot2_y = temp_messptc10_3A
#### for the Temp nocontrol plot
plot1_x = time_messptc10
plot1_y = temp_messptc10_3A
plot2_x = time_messptc10
plot2_y = temp_messptc10_3C
# #### for the Press plot
# plot1_x = time_cpg2500
# plot1_y = press_cpg2500
# plot2_x = time_baro_room
# plot2_y = press_baro_room
plot3_x = time_ptc10
...
# std_coll = 'collimator (air) temperature (PV: {:.4f} K, {}{:.4f} K)'.format(pv_plot2, sigma, std_plot2)
#### for the Temp nocontrol plot
std_coll = 'collimator (air) temperature (PV: {:.4f} K, {}{:.4f} K)'.format(pv_plot1, sigma, std_plot1)
std_tank = 'tank (air) temperature (PV: {:.4f}  K, {}{:.4f} K)'.format(pv_plot2, sigma, std_plot2)
# #### for the Press plot
# std_cpg = 'tank pressure (PV: {:.2f} hPa, {}{:.2f} hPa)'.format(pv_plot1, sigma, std_plot1)
# std_wut_room = 'room pressure (PV: {:.1f} hPa, {}{:.1f} hPa)'.format(pv_plot2, sigma, std_plot2)
...
#### for the Temp nocontrol plot
p1, = ax.plot_date(plot1_x, plot1_y, fmt='-', color='tab:orange', alpha=1.0, linewidth=2.5, label=std_coll)
p2, = ax2.plot_date(plot2_x, plot2_y, fmt='-', color='darkblue', alpha=0.8, linewidth=2.5, label=std_tank)
# #### for the Press plot
# p1, = ax.plot_date(plot1_x, plot1_y, fmt='-', color='tab:orange', alpha=1.0, linewidth=2.5, label=std_cpg)
# p2, = ax2.plot_date(plot2_x, plot2_y, fmt='-', color='darkblue', alpha=0.8, linewidth=2.5, label=std_wut_room)
...
# #### ONLY for the Temp plot WITH control
...
plt.setp(ax.get_yticklabels(), color=p1.get_color(), fontsize=20, alpha=1.0)
plt.setp(ax2.get_yticklabels(), color=p2.get_color(), fontsize=20)
plt.setp(ax3.get_yticklabels(), color=p3.get_color(), fontsize=20)
...
plt.legend(lines + lines2 + lines3, labels + labels2 + labels3, loc='center', bbox_to_anchor=(0.5, 1.15), fontsize=22)
...
plt.draw()
plt.savefig('/mnt/c/Users/Bananaking/Documents/Vanessa/PhDThesis/plotting_stuff/plots/Temp_noregulation.pdf')
plt.show()
plt.close(fig)
