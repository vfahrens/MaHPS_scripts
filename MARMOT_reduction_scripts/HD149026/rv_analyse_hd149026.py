...
def main(inifile=None):
    ...
    if True:
        ...
        if True:
            ...
            select_use_frames=ana.select_template_frames(plot=True,mode='single',quantity=4)
            ...
            #ana.apply_comb_wv_cal(orders=ana.order_list, pixrange=[200,1850], use_frames=ana.double_frames, which='cal', CombAsComb=False, plotwv=False, multicore=ana.multicore)

            pickle.dump(ana, open(os.path.join(ana.datainf['opath'], ana.datainf['analyproj']), "wb"))
            ana.rv_ccf(fiber='A', order_list=ana.order_list, fitrange=[200, 1850], use_frames=all_frames, opath=ana.datainf['opath'], show=False, multicore=ana.multicore, remove_tellurics=False)
            pickle.dump(ana, open(os.path.join(ana.datainf['opath'], ana.datainf['analyproj']), "wb"))

            
            ### single order RVs saved to file ###
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3207_singleord_marmot_ccffit.txt'), 'w') as single_file:
            ...
            ### single order RVs saved to file ###
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3207_singleord_marmot_ccf.txt'), 'w') as single_file:
                for gg in all_frames:
                    fileid_s = int(ana.frames[gg].filename[:8]+ana.frames[gg].filename[9:13])
                    date_jd_s = ana.frames[gg].midexpt.jd
                    for ordi in ana.order_list:
            ...
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3207_weighted_marmot_ccffit.txt'), 'w') as rv_file:
            ...
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3207_weighted_marmot_ccf.txt'), 'w') as rv_file:
                for fif in all_frames:
                    fileid = int(ana.frames[fif].filename[:8]+ana.frames[fif].filename[9:13])
                    date_jd = ana.frames[fif].midexpt.jd
