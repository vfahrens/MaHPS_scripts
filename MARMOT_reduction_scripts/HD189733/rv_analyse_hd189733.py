...
def main(inifile=None):
    ...
    if True:
        ...
        if True:
            ...
            select_use_frames=ana.select_template_frames(plot=True,mode='single',quantity=3)
            ...
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3155_singleord_marmot_ccf.txt'), 'w') as single_file:
            ...
            ### single order RVs saved to file ###
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3155_singleord_marmot_ccffit.txt'), 'w') as single_file:
                for gg in all_frames:
                    fileid_s = int(ana.frames[gg].filename[:8]+ana.frames[gg].filename[9:13])
                    date_jd_s = ana.frames[gg].midexpt.jd
                    for ordi in ana.order_list:
            ...
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3155_weighted_marmot_ccf.txt'), 'w') as rv_file:
            ...
            with open(os.path.join(ana.datainf['opath'], 'RVs_ID3155_weighted_marmot.txt'), 'w') as rv_file:
                for fif in all_frames:
                    fileid = int(ana.frames[fif].filename[:8]+ana.frames[fif].filename[9:13])
                    date_jd = ana.frames[fif].midexpt.jd
